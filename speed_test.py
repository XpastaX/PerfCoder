...
def report_results(df, cfg, orig_df): 
        ...
        for i, row in df.iterrows():
            for j in range(num_generated_cols):
                colname = f"{cfg.model_generated_potentially_faster_code_col}_{j}" if num_generated_cols > 1 else cfg.model_generated_potentially_faster_code_col
            ...
            row["fastest_generated_agg_runtime"] = min([row[f"{colname}_agg_runtime_adjusted"] for j in range(num_generated_cols)])
            new_rows.append(row)
...
def read_inputs_and_prepare_v2(cfg) -> pd.DataFrame:
    ...
    print(f"Loading data from {cfg.model_generated_outputs_path}")
    gen_df = pd.read_json(
...
def main(cfg):
    ...
    # print(cfg.reference_file_path)
    with open(cfg.reference_file_path, 'r') as f:
        reference_df = pd.read_json(f, lines=True, orient="records")
    ...
    if not os.path.exists(os.path.join(cfg.output_dir, "test_results.jsonl")):
        ...
        try: 
            ...
            global env
            # env = simulator.make(timeout_seconds_gem5=120, verbose=True, use_logical_cpus=True, port=10085, workers=30, exit_early_on_fail=True)
            env = simulator.make(timeout_seconds_gem5=120, verbose=False, use_logical_cpus=True, port=10085, workers=30, exit_early_on_fail=True)
            ...
            code_list = batch["code"].tolist()
            test_sorted = [sorted(list(t), reverse=True) for t in batch["tests"].tolist()]
            results = env.submit_multiple_single_submissions(code_list,
                                                            test_sorted,
        ...
        read_path = f"{cfg.output_dir}/test_results.jsonl"
        unmelted_df = pd.read_json(read_path,
            orient="records",
            lines=True
        )
        print(f'Load previous results from {read_path}')
...
def merge_config_into_namespace(args: argparse.Namespace, yaml_config: dict) -> argparse.Namespace:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_generated_outputs_path", type=str)
    parser.add_argument("--output_dir", type=str)
    parser.add_argument("--reference_file_path", type=str)
    parser.add_argument("--is_prompt_based", type=bool, default=False)
    parser.add_argument("--model_generated_potentially_faster_code_col", type=str, default="response")
    parser.add_argument("--slow_code_col", type=str, default="src_code")
    parser.add_argument("--reference_code_col", type=str, default="tgt_code")
    parser.add_argument("--cpuset_cpus", type=str, default=None)
    parser.add_argument("--do_eval", type=bool, default=False)
    parser.add_argument("--cpus_available", type=int, default=1)
    parser.add_argument("--num_problems_to_evaluate", type=int, default=-1)
    parser.add_argument("--threshold_accuracy", type=float, default=1.0)
    parser.add_argument("--redo_src_tgt", action='store_true')
    parser.add_argument("--num_generated_cols", type=int, default=None)

    args = parser.parse_args()

    # Path for this project, auto extract fast colum
    if args.model_generated_potentially_faster_code_col == 'auto':
        from utils.common import load_data
        _data = load_data(args.model_generated_outputs_path)[0]
        for key in _data:
            if '_resp' in key and 'qwen2.5' in key:
                args.model_generated_potentially_faster_code_col = key.strip('_resp')
                break
        print(f"The new faster column is {args.model_generated_potentially_faster_code_col}")

    if args.config_path:
        yaml_config = load_config(args.config_path)
        args = merge_config_into_namespace(args, yaml_config)

    # Optional: remove config_path if you don't want to pass it
    delattr(args, "config_path")
