...
In the PerfCoder paper, this gem5 wrapper is used in two places:

* during **evaluation** (`speed_test.py` → `gem5/gem5_eval.py`) to obtain the per-program (accuracy, runtime, speedup) triples reported in Table 2 / Table 3;
* during **GRPO training**, as the reward provider behind `src/pie/gem5_client.py` (`reward = compile_ok · accuracy · clip(speedup)`).
